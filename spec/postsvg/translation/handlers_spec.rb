...
RSpec.describe Postsvg::Translation::Handlers do
  ...
  it "RectHandler emits gsave/moveto/rlineto×3/closepath/paint/grestore" do
    ...
    ops = classes(program)
    expect(ops).to include(Postscript::Model::Operators::GraphicsState::Gsave)
    expect(ops).to include(Postscript::Model::Operators::Path::Moveto)
    expect(ops).to include(Postscript::Model::Operators::Path::Closepath)
    expect(ops).to include(Postscript::Model::Operators::Painting::Fill)
    expect(ops).to include(Postscript::Model::Operators::GraphicsState::Grestore)
  end
  ...
  it "CircleHandler emits Arc with radius and 0/360 angles" do
    ...
    arc = program.body.find { |o| o.is_a?(Postscript::Model::Operators::Path::Arc) }
  ...
  it "LineHandler emits moveto/lineto/stroke" do
    ...
    ops = classes(program)
    expect(ops).to include(Postscript::Model::Operators::Path::Moveto)
    expect(ops).to include(Postscript::Model::Operators::Path::Lineto)
    expect(ops).to include(Postscript::Model::Operators::Painting::Stroke)
  end
  ...
  it "PolygonHandler emits closepath (Polyline does not)" do
    ...
    expect(classes(polygon)).to include(Postscript::Model::Operators::Path::Closepath)

    polyline = translate(%(<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><polyline points="10,10 50,90 90,10"/></svg>))
    expect(classes(polyline)).not_to include(Postscript::Model::Operators::Path::Closepath)
  end
  ...
  it "PathHandler translates cubic bezier C" do
    ...
    expect(classes(program)).to include(Postscript::Model::Operators::Path::Curveto)
  end
  ...
  it "GroupHandler wraps children with gsave/grestore" do
    ...
    expect(classes(program)).to include(Postscript::Model::Operators::GraphicsState::Gsave)
    expect(classes(program)).to include(Postscript::Model::Operators::GraphicsState::Grestore)
  end
